...
export const ProfilePage = () => {
  ...
  return (
    <>
      ...
      <div className="container-shell section-space space-y-10">
        <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
          <div className="surface-panel p-6 sm:p-8">
            <p className="eyebrow">{dictionary.profile.title}</p>
            <h1 className="mt-3 font-heading text-4xl font-bold tracking-tight">{profile.fullName}</h1>
            <p className="mt-3 text-sm text-neutral-500">{profile.email}</p>
                    ...
                    <div className="mt-5 space-y-3">
                      {order.items.map((item) => (
                        <div key={item.id} className="flex items-center justify-between gap-3 text-sm">
                          <span>{item.productName[locale]} x {item.quantity}</span>
